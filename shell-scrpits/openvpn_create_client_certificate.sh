...
create_client_conf(){ #创建客户端client.conf文件
	IP=`grep "local" ${script_dir}/config/server.conf| grep -Eo '((25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]?)'`
...
EOF
	...
	echo
	echo "启动方法：cd $config_path ; $client_path/sbin/openvpn server.conf &"
}
